...
class Model_DbTable_Destination extends Zend_Db_Table_Abstract
{
  ...
  public function findStatusesCount($user_id = null)
  {
    $select = $this->select()
            ...
            ->join($this->_description,
                    "{$this->_name}.poi_id = {$this->_description}.poi_id")
  ...
  public function findAllWithDescription($language_id, $filter = null, $user_id = null)
  {
    $select = $this->select()->setIntegrityCheck(false)
            ->from($this->_name,
                    array('status', 'poi_id', 'approved_at', 'viewer', 'image', 'main_category', 'area' => "(GROUP_CONCAT({$this->_area}.name SEPARATOR ', '))"))
            ->join($this->_description,
                    "{$this->_name}.poi_id = 
                    {$this->_description}.poi_id",
                    array('name', 'created_at', 'updated_at'))
            ...
            ->joinLeft($this->_category,
                    "{$this->_name}.main_category = {$this->_category}.category_id",
                    array(
                'main_category' => 'name'
            ))
            ->joinLeft(array('created_by' => $this->_account),
                    "{$this->_description}.created_by = created_by.admin_id",
                    array('created_by' => 'username'))
            ->joinLeft(array('updated_by' => $this->_account),
                    "{$this->_description}.updated_by = updated_by.admin_id",
            ...
            ->join($this->_cultureArea,
                    "{$this->_name}.poi_id = {$this->_cultureArea}.poi_id",
                    array())
            ->join($this->_area,
                    "{$this->_cultureArea}.area_id = {$this->_area}.area_id",
                    array())
    ...
    if (null != $filter) {
      if (null != $filter['name']) $select->where("{$this->_description}.name LIKE ?",
                "%{$filter['name']}%");
      if (null != $filter['status']) $select->where("status = ?",
                $filter['status']);
      if (null != $filter['sort']) {
        $select->order("{$filter['sort']} {$filter['order']}");
      }
      if (0 != $filter['main_category']) {
        $select->where("{$this->_name}.main_category = ?",
  ...
  public function updateRating($poiId, $input, $oldRate = '', $newRate = '')
  {
    ...
    return array('rating'     => number_format($data['totalrate'] / $data['totalrater'],
                2),
  ...
  public function getAllWithDesc()
  {
    $select = $this->select()
            ...
            ->join(array('pd' => $this->_poidesc),
                    'p.poi_id = pd.poi_id')
            ->join(array('l' => 'language'),
            'pd.language_id = l.language_id', array('language_name'));
...
//      echo $query->__toString();
  ...
  public function getAllWithDescForRss($lang_id = 1, $limit = '')
  {
    $query = $this->select()
            ->from($this->_name)
            ->join($this->_poidesc,
                    "{$this->_poidesc}.poi_id = {$this->_name}.poi_id")
  ...
  public function getAllByIdLang($poi_id, $language_id, $isPublish = true, $isApproved = false)
  {
    ...
    $query = $this->select()
            ->from($this->_name,
                    array('poi_id', 'pointX', 'pointY', 'address', 'phone', 'website',
...
//                echo $query->__toString();
  ...
  public function getAllByName($name, $language_id = 1)
  {
    $data = $this->select()
            ...
            ->join(array('pd' => 'poidescription'),
                    'p.poi_id = pd.poi_id', array('name'))
  ...
  public function getNameIdMainCatById($poi_id)
  {
    $data = $this->select()
            ->from($this->_name,
                    array('poi_id', 'name', 'main_category'))
            ->where('poi_id = ?', $poi_id);
    return $this->fetchAll($data);
  }
  ...
  public function getImageNameDescriptionById($poiId, $langId)
  {
    $data = $this->select()
            ...
            ->join($this->_poidesc,
                    ...
                    array('description'))
            ->join($this->_gallery,
                    "{$this->_gallery}.poi_id =
                {$this->_poidesc}.poi_id",
                    array('source'))
            ->setIntegrityCheck(false)
            ->where("{$this->_poidesc}.language_id = ?", $langId)
  ...
  public function getAllImageNameDescription()
  {
    $data = $this->select()
            ...
            ->join($this->_poidesc,
                    ...
                    array('description'))
            ->join($this->_gallery,
                    "{$this->_gallery}.poi_id = {$this->_name}.poi_id",
                    array('source'))
            ->setIntegrityCheck(false)
            ->where("{$this->_poidesc}.language_id = 1");
    return $this->fetchAll($data);
  ...
  public function getQueryAllByLang($filter = '0', $param = null, $language_id = 1, $author_id = null)
  {
    ...
    $select = $this->select()
            ...
            ->join(array('poides' => 'poidescription'),
                    'poides.poi_id = poi.poi_id', array('name'))
    ...
    switch ($filter) {
      case 1 : $select->where("poides.name LIKE '%" . $param . "%'");
        break;
      case 2 : $select->join('areatopoi',
                'areatopoi.poi_id = poi.poi_id');
        $select->where('areatopoi.area_id = ?', $param);
        break;
      case 3 : $select->join(array('ctp' => 'categorytopoi'),
                'ctp.poi_id = poi.poi_id', array());
  ...
  public function getAuthor($poi_id, $languageId)
  {
    $query = $this->select()
            ...
            ->join($this->_account,
                    "{$this->_poidesc}.created_by = {$this->_account}.admin_id")
            ->where("{$this->_poidesc}.poi_id = ?", $poi_id);
    $data = $this->fetchAll($query);
    return $data;
  }
  ...
  public function getRelatedCulture($id, $languageId)
  {
    $query = $this->select()
            ->setIntegrityCheck(false)
            ->from($this->_poidesc)
            ->join($this->_categorytopoi,
                    "{$this->_poidesc}.poi_id = {$this->_categorytopoi}.poi_id")
            ->join($this->_name,
                    "{$this->_name}.main_category = {$this->_categorytopoi}.category_id ")
