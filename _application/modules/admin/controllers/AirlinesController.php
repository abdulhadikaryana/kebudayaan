...
class Admin_AirlinesController extends Library_Controller_Backend
{
    /**
     * IS: -
     * FS: -
    ...
    public function indexAction()
    {
        ...
        $select =  $airlines->getAllQuery($filter, $param);
    ...
    public function createAction()
    {
        ...
        if($this->getRequest()->isPost())
        {
            if($form->isValid($_POST))
            {
                ...
                $this->_redirect($this->view->rootUrl('/admin/airlines/'));
            }
        }
        $this->view->form = $form;
    }
    ...
    public function editAction()
    {
        ...
        if($this->getRequest()->isPost())
        {
            if($form->isValid($_POST))
            {
                if($language_id!=1)
                {
                    $indo = $airlinedesc->checkForIndo($air_id);
                    if($indo)
                    {
                        $data2 = array(
                                'airline_id' => $air_id,
