...
class Admin_RelatedlinksController extends Library_Controller_Backend {
    ...
    if ($this->getRequest()->isPost()) {
      if ($form->isValid($_POST)) {
        ...
        if (!empty($related_id)) {
          ...
          $related_id = $table_reldesc->insertDataRel($data2);

          $this->loggingaction('relatedlinks', 'create', $related_id);
          $this->_flash->addMessage('1\Related Links Insert Success!');
        } else {
          $this->_flash->addMessage('2\Related Links Insert Failed!');
        }
        $this->_redirect($this->view->rootUrl('/admin/relatedlinks/'));
      }
    }
    $this->view->form = $form;
    $this->view->langId = 1;
  ...
  public function editAction() {
    ...
    if ($this->getRequest()->isPost()) {
      if ($form->isValid($_POST)) {
        if ($langId != 1) {
          $indo = $table_reldesc->checkForIndo($related_id, 2);
          if ($indo == false) {
            $data = array(
                'id' => $related_id,
                'language_id' => 2,
            ...
            $table_reldesc->updateRelatedIndo($data2, $related_id, 2);
          }
        } else {
          $data = array(
              'jenisrelated' => $_POST['linkType'],
        ...
        $this->loggingaction('relatedlinks', 'edit', $related_id);
        $this->_flash->addMessage('1\Related Links Update Success!');
        $this->_redirect($this->view->rootUrl('/admin/relatedlinks/'));
      }
    }
