...
class Admin_VitoindoController extends Library_Controller_Backend {
  ...
  public function indexAction() {
    ...
    $select = $table_tourismoperator->getQueryVitoByLang($filter, $param, 2);
    ...
    if ($this->getRequest()->isPost()) {
      if ($form->isValid($_POST)) {
        ...
        if (!empty($tourism_id)) {
          ...
          $this->_flash->addMessage('1\Vito Indo Insert Success!');
        } else {
          $this->_flash->addMessage('2\Vito Indo Operator Insert Failed!');
        }
        $this->_redirect($this->view->rootUrl('/admin/vitoindo/'));
      }
    }
  ...
  public function editAction() {
    ...
    if ($this->getRequest()->isPost()) {
      if ($form->isValid($_POST)) {
        ...
        $this->_flash->addMessage('1\Vito Indo Update Success!');
        $this->_redirect($this->view->rootUrl('/admin/vitoindo/'));
      }
    }

    $data_amount = $table_classtotourism->countClassByTourismId($tourism_id);
    $data = $table_tourism->getAllTourismDataByIdLang($tourism_id, 2);
