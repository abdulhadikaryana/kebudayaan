...
class Model_DbTable_Event extends Zend_Db_Table_Abstract {
    ...
    public function findAll($filter = null, $language_id = self::LANGUAGE_ID, $user_id = null) {
        $select = $this->select()
                ...
                ->from($this->_name)
                ->join($this->_table, "{$this->_name}.event_id = {$this->_table}.event_id", array('name'))
                ->columns(array('isTranslated' => "(
              SELECT COUNT(*) 
              FROM {$this->_table} 
              WHERE event_id = {$this->_name}.event_id
              AND language_id = 2)"))
                ->joinLeft($this->_category, "{$this->_name}.category = {$this->_category}.category_id", array('category' => 'name'))
                ->joinLeft(array('created_by' => $this->_account), "{$this->_name}.user_id = created_by.admin_id", array('created_by' => 'username'))
                ->joinLeft(array('updated_by' => $this->_account), "{$this->_name}.updated_by = updated_by.admin_id", array('updated_by' => 'username'))
        ...
        if (null != $filter) {
            if (null != $filter['name']) {
                $select->where("{$this->_table}.name LIKE ?", "%{$filter['name']}%");
            }
            if (null != $filter['status']) {
                $select->where("{$this->_name}.status = ?", $filter['status']);
            }
            if (null != $filter['sort']) {
                if ($filter['sort'] == 'time_created') {
                    $select->order(array("updated_at {$filter['order']}", "time_created {$filter['order']}"));
                } else {
    ...
    public function getQueryAllByLang($filter, $param1 = null, $param2 = null, $language_id = 1, $user_id = null) {
        $select = $this->select()
                ->setIntegrityCheck(FALSE)
                ->from(array('e' => $this->_name), array('event_id', 'dstart' => 'DATE(date_start)', 'dend' => 'DATE(date_end)', 'category', 'date_start', 'date_end', 'main_pics', 'status'))
                ->join(array('ed' => $this->_table), 'ed.event_id = e.event_id', array('name', 'description'))
                ->join(array('cd' => 'categorydescription'), 'cd.category_id = e.category', array('category_name' => 'cd.name'))
        ...
        switch ($filter) {
            case 1 : $select->where("ed.name LIKE '%" . $param1 . "%'");
                break;
            case 2 : $select->join(array('pte' => 'poitoevent'), 'pte.event_id = e.event_id', array('poi_id'));
                $select->join(array('pd' => 'poidescription'), 'pd.poi_id = pte.poi_id', array());
...
//        if($process_date)
    ...
    public function getAllWithDesc($lang_id = 1) {
        $data = $this->select()
                ->from($this->_name)
                ->join($this->_table, "{$this->_table}.event_id = {$this->_name}.event_id")
                ->join($this->_account, "{$this->_name}.user_id = {$this->_account}.admin_id", array(
                    'created_by' => "IFNULL({$this->_account}.name, username)"
                ))
                ->setIntegrityCheck(false)
    ...
    public function getThreeLatestEvent($languageID = 1) {
        $data = $this->select()
                ->from($this->_name, array('event_id', 'date_start', 'date_end', 'main_pics'))
                ->join($this->_table, "{$this->_table}.event_id = {$this->_name}.event_id", array('name', 'description'))
                ->setIntegrityCheck(false)
    ...
    public function getFourClosestEvent($languageID = 1) {
        ...
        $query = $this->select()
                ->distinct()
                ->setIntegrityCheck(false)
                ->from($this->_name, array('event_id', 'date_start', 'date_end', 'main_pics', $noSignDiff))
                ->join($this->_table, "{$this->_table}.event_id = {$this->_name}.event_id", array('name', 'description'))
                ->where("{$this->_table}.language_id = ?", $languageID)
...
//        return $sortBy . ' ' . $sortOrder;
    ...
    public function api_getAllWithDesc($param) {
        $data = $this->select()
                ...
                ->join(array('ed' => $this->_table), 'ed.event_id = e.event_id')
    ...
    public function api_getAllWithoutDesc($param) {
        $query = $this->select()
                ->setIntegrityCheck(false)
                ->from(array('e' => $this->_name), array('event_id', 'time_created', 'date_start', 'date_end', 'viewer'))
                ->join(array('ed' => $this->_table), 'ed.event_id = e.event_id', array('language_id', 'name', 'description'))
...
//
    ...
    public function getAuthor($id, $languageId) {
        $query = $this->select()
                ->setIntegrityCheck(false)
                ->from($this->_name)
                ->join($this->_account, "{$this->_name}.user_id = {$this->_account}.admin_id")
    ...
    public function getLatestPost($limit) {
        $query = $this->select()
                ->setIntegrityCheck(false)
                ->from($this->_table)
                ->join($this->_name, "{$this->_table}.event_id = {$this->_name}.event_id")
                ->join($this->_account, "{$this->_name}.user_id = {$this->_account}.admin_id", array('admin_name' => 'name'))
                ->where("{$this->_name}.status = '3'")
