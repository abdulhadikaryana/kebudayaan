...
class EventController extends Budpar_Controller_Common {
    ...
    public function indexAction() {
        ...
        $this->view->headLink()->headLink(array(
            'rel' => 'canonical',
            'href' => Zend_Controller_Front::getInstance()->getRequest()->getScheme() . '://' . Zend_Controller_Front::getInstance()->getRequest()->getHttpHost() . $this->view->url(array(), $this->_languageId == 1 ? 'kegiatan' : 'event', true)
                ), 'PREPEND');
    }
    ...
    public function detailAction() {
        ...
        // Data
        $event = $eventDb->getAllWithDescById($id, $this->_languageId)->toArray();
        $author = $eventDb->getAuthor($id, $this->_languageId);

        if (empty($event)) {
            // Data
            $event = $eventDb->getAllWithDescById($id, 1);
            $author = $eventDb->getAuthor($id, 1);
        }
