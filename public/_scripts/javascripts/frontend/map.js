...
lamanbudaya.maps = {
    ...
    createContent : function(data) {
        ...
        var img = '';        
        if(data.image != '') {
            img = '<img class="alignleft" src="' + lamanbudaya.url.imageUrl + '/timthumb.php?src=upload/culture/' + data.image + '&w=150&h=150&q=100" alt="" />';
        }
        content += '<h4>' + data.name + '</h4>' + img + '<p>' + data.description + '<span class="read-more"><a class="pointer" onclick="window.open(\'' + lamanbudaya.url.baseUrl + '/culture/' + data.id + '\')">' + read_more + ' &gt;&gt;</a></span>' + '</p>';
        content += '<div class="clear-left"></div>';
        content += '<h5>' + categoryText + '</h5>';
...
function FullMapControl(controlDiv) {
    ...
    controlUI.title = 'Klik untuk melihat peta indonesia secara menyeluruh';
    ...
    controlText.innerHTML = 'Peta Indonesia';
