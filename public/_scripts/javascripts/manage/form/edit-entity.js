...
function prepareData(arr, formObj, options)
{
	...
	{
        description[$('#language').val()] = tinyMCE.get('description').getContent({format : 'raw'});
	}
...
$(function(){
    ...
    $.ajax({
        ...
        success: function(data){
			...
			$('#language > option[value="'+data.description[0].language_id+'"]').attr('selected','selected');
            ...
            if('description' in data)
            {
                ...
                if(description[languageId] != undefined)
                {
                    if(tinyMCE.editors.length > 0)
                    {
                        tinyMCE.activeEditor.setContent(description[languageId]);
                    }
                    else
                    {
                        $('#description').val(description[languageId]);
                    }
                }
            }
