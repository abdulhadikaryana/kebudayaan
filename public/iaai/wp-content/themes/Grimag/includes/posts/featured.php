<?php if ( !defined( 'ABSPATH' ) ) exit;
	...
	if ( $st_['featcount'] == 1 ) {
		...
		echo '<div class="posts-featured-' . ( $wp_query->found_posts == 1 ? 'alone' : 'a' ) . '-wrapper">';
			...
			if ( has_post_thumbnail() ) {
				...
				$st_['thumb'] = wp_get_attachment_image_src( $st_['id'], $wp_query->found_posts == 1 ? 'large' : 'project-medium' );
			...
			// Compose thumb
			echo '<a href="' . get_permalink() . '" class="post-thumb" ' . ( function_exists( 'st_get_2x' ) ? st_get_2x( $post->ID, $wp_query->found_posts == 1 ? 'large' : 'project-medium', 'attr' ) : '' ) . ' style="background-image: url(' . $st_['thumb'] . ')" data-format="' . $st_['format'] . '">&nbsp;</a>';
