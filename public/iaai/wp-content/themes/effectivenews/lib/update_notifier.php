...
function update_notifier_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
	    $xml = get_latest_theme_version(21600); // Get the latest remote XML file on our server
            ...
            if( version_compare($xml->latest, $theme_data['Version'], '>') ) { // Compare current theme version with the remote XML version
				...
				add_submenu_page('EffectiveLab', $theme_data['Name'] . ' Theme Updates', $theme_data['Name'] . ' <span class="update-plugins count-1"><span class="update-count">1 Update</span></span>','administrator', 'theme-update-notifier' , 'update_notifier');
			}
	}
}
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( version_compare($xml->latest, $theme_data['Version'], '>') ) { // Compare current theme version with the remote XML version
			$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . $theme_data['Name'] . ' <span id="ab-updates">1 Update</span></span>', 'href' => get_admin_url() . 'admin.php?page=theme-update-notifier' ) );
		}
	}
}
...
<div class="wrap">
  ...
  <h2>
    <?php echo strtolower($theme_data['Name']); ?>
    Theme Updates</h2>
  ...
  <div id="instructions">
    ...
    <p><strong>Please note:</strong> make a <strong>backup</strong> of the Theme inside your WordPress installation folder <code>/wp-content/themes/<?php echo strtolower($theme_data['Name']); ?>/</code></p>
    <p>
    <h3>DOWNLOAD</h3>
    ...
    <p>
    <h3>UPDATE</h3>
    Upload the new files using FTP to the <code>/wp-content/themes/<?php echo strtolower($theme_data['Name']); ?>/</code> folder overwriting the old ones .
    </p>
...
function get_latest_theme_version($interval) {
	$notifier_file_url = 'http://theme.effectivelab.net/effnews/notifier.xml';
