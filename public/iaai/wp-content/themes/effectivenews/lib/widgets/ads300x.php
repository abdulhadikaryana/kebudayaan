...
add_action('widgets_init','eff_ads300x');

function eff_ads300x() {
	register_widget('eff_ads300x');
	
	}

class eff_ads300x extends WP_Widget {
	function eff_ads300x() {
			
		$widget_ops = array('classname' => 'eff_ads300x','description' => __('Widget display 300*100 Ads','framework'));
		$this->WP_Widget('eff_ads300x',__('Widget - Ads 300*100','framework'),$widget_ops);
...
function form( $instance ) {
		...
		$defaults = array( 'title' => __('advertisement', 'framework'), 
			'img' => EFF_IMG.'/ads300x.png', 
			'link' => '#',
			'img2' => EFF_IMG.'/ads300x.png', 
			'link2' => '#'
