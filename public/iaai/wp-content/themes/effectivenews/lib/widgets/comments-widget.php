...
add_action('widgets_init','eff_recent_comments');

function eff_recent_comments() {
	register_widget('eff_recent_comments');
	}

class eff_recent_comments extends WP_Widget {
	function eff_recent_comments() {
			
		$widget_ops = array('classname' => 'eff_comments','description' => __('Widget display Recent Comments with avatar','framework'));
		
		$this->WP_Widget('eff_comments',__('Widget - Recent Comments','framework'),$widget_ops);
...
?>
		...
		<ul class="post_list">
				...
				?>
                        <li>
                        <a class="av_thumb" href="<?php echo get_permalink($comment->ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php echo strip_tags($comment->comment_author); ?> <?php _e('on ', 'framework'); ?><?php echo $comment->post_title; ?>"><?php echo get_avatar( $comment, '55', $default=eff_option('custom_gravatar') ); ?></a>
