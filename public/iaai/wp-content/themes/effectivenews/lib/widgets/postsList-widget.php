...
add_action('widgets_init','eff_widget_posts_list');

function eff_widget_posts_list() {
	register_widget('eff_widget_posts_list');
	
	}

class eff_widget_posts_list extends WP_Widget {
	function eff_widget_posts_list() {
			
		$widget_ops = array('classname' => 'posts_list','description' => __('Widget display Posts order by : Popular, Random, Recent','framework'));
		$this->WP_Widget('eff-posts_list',__('Widget - Posts List','framework'),$widget_ops);
