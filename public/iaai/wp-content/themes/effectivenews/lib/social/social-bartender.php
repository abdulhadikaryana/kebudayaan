...
*/
define( 'EFF_SOC', EFF_URI . '/lib/social/');

add_action('init',  'sh_sb_init');

function sh_sb_init(){

	load_plugin_textdomain( 'framework', FALSE, EFF_SOC.'languages' );
...
function sh_sb_create_menu(){
	...
	$sh_sb_settings = add_submenu_page('themes.php', __('Social Icons','framework'), __('Social Icons','framework'), 'manage_options', 'sh_sb_settings_page', 'sh_sb_settings_page');
...
function sh_sb_image_admin_scripts() { 
	wp_register_script( 'social-bartender-scripts', EFF_SOC.'js/scripts.js', 
...
function sh_sb_image_admin_styles() { 
	if( version_compare( '3.3', get_bloginfo( 'version' ), '>' ) ):
		wp_register_style( 'social-bartender-css', EFF_SOC.'css/styles.css', array( 'thickbox', 'nav-menu' ) );
	else:
		wp_register_style( 'social-bartender-css', EFF_SOC.'css/styles.css', array( 'wp-admin', 'thickbox' ) );
	endif;
...
function social_bartender( $link_before = '', $link_after = '', $echo = 1 ){
	...
	foreach( $items as $item ):
		...
		if( $link ):
			...
			$output .= '<li><a href="'.$link.'" class="'.$title.' tip_n" title="'.$title.'">';
...
require_once( EFF_FW . "/social/settings.php");
require_once( EFF_FW . "/social/help.php");
require_once( EFF_FW . "/social/widget.php");
