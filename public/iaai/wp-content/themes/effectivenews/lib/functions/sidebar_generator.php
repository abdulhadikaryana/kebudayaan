...
class sidebar_generator {
	...
	function sidebar_generator(){
		...
		//edit cat
		//add_action('category_add_form_fields', array('sidebar_generator', 'edit_form_cat'));
		//add_action('category_edit_form_fields', array('sidebar_generator', 'edit_form_cat'));
		...
		//save cat
		//add_action('edited_category', array('sidebar_generator', 'save_form_cat'));
		//add_action('create_category', array('sidebar_generator', 'save_form_cat'));
	...
	*/
	...
	function save_form_cat($term_id){
		if (!isset($_POST['sbg_edit']))  $_POST['sbg_edit'] = ''; 
		$is_saving = $_POST['sbg_edit'];
		if(!empty($is_saving)){
			delete_post_meta($term_id, 'sbg_selected_sidebar_replacement');
			add_post_meta($term_id, 'sbg_selected_sidebar_replacement', $_POST['sidebar_generator_replacement']);
		}
	...
	function edit_form_cat(){
	    $term_id = $tag->term_id;
	    $term_meta = get_option( "taxonomy_$term_id" );
	    if (is_object($term_id)) {
	    	$term_id = $tag->term_id;
	    }
	    $selected_sidebar = get_option($term_id, 'sbg_selected_sidebar', true);
	    if(!is_array($selected_sidebar)){
	    	$tmp = $selected_sidebar; 
	    	$selected_sidebar = array(); 
	    	$selected_sidebar[0] = $tmp;
	    }
	    $selected_sidebar_replacement = get_option($term_id, 'sbg_selected_sidebar_replacement', true);
		if(!is_array($selected_sidebar_replacement)){
	    	$tmp = $selected_sidebar_replacement; 
	    	$selected_sidebar_replacement = array(); 
	    	$selected_sidebar_replacement[0] = $tmp;
		...
		<tr class="form-field">
		<th scope="row" valign="top"><label><?php _e('Sidebars'); ?></label></th>
			<input name="sbg_edit" type="hidden" value="sbg_edit" />
